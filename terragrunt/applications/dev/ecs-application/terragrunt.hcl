...
dependency "vpc" {
  config_path                             = "${get_parent_terragrunt_dir("root")}/base-infrastructure/${include.stage.locals.stage}/vpc"
...
dependency "ecs_cluster" {
  config_path                             = "${get_parent_terragrunt_dir("root")}/base-infrastructure/${include.stage.locals.stage}/ecs-cluster"
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
...
dependency "alb" {
  config_path                             = "${get_parent_terragrunt_dir("root")}/base-infrastructure/${include.stage.locals.stage}/alb"
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
...
inputs = {
  ...
  ecs_service = {
    ...
    launch_type     = "FARGATE"
    desired_count   = 3
    egress_all_id   = dependency.alb.outputs.aws_sg_egress_all_id
    private_subnets = dependency.vpc.outputs.vpc_private_subnets_ids
  }

  vpc_id  = dependency.vpc.outputs.vpc_id
  alb_arn = dependency.alb.outputs.aws_alb_arn
}

terraform {
  source = "${get_parent_terragrunt_dir("root")}/..//terraform/ecs-application"
}